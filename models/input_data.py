...
def extract_labels(filename, one_hot=False):
  ...
  if one_hot:
    tagToInt = lambda tag: {'O': 0, 'PER': 1, 'ORG': 2, 'LOC': 3, 'MISC': 4}[tag]
    entities = [tagToInt(tag) for tag in entities]
...
def read_data_sets(train_dir, n_labeled = 100, fake_data=False, one_hot=False):
  ...
  word_vectors = extract_words('./corpus_WiNER/word_vectors/wv_sample_exp_decay_W_5.npz')

  # local_file = maybe_download(TRAIN_LABELS, train_dir)
  entities = extract_labels('./corpus_WiNER/entity_vectors/ev_sample_exp_decay_W_5.npz',
                              one_hot=one_hot)
