...
#     sen_matrix, count = fill_w2vMatrix(sentence, sen_matrix, D, w2v_model)
...
def exponential_decay(sentence, D, W, w2v_model):
    ...
    sen_matrix, count, no_vocab_list = fill_w2vMatrix(sentence, sen_matrix, D, w2v_model)    
    ...
    for I in range(L):
        vectors[I,:] = np.sum([sen_matrix[:,j] * ((1-alpha)**(abs(I-j)-1)) 
                               for j in range(I - W, I + W + 1) 
                                if j >= 0 and j != I and j < L], axis=0)       
    # count: amount of words that are not in the vocabulary. They are transformed to zeros vectors.
    return vectors, count, no_vocab_list
